    ...
    async def study(self, interaction: discord.Interaction):import discord
from discord.ext import commands
from discord import app_commands  # <--- THIS WAS MISSING!
import aiohttp
import os
import asyncio
...
class HighTech(commands.Cog):
    ...
    # --- FEATURE 1: AI CHATBOT (Powered by Hugging Face) ---
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(API_URL, headers=headers, json=payload) as resp:
                    if resp.status == 200:
                        ...
                        print(f"HF API Error: {error_text}")  # Print to Render logs for debugging
...
class HighTech(commands.Cog):
    ...
    async def chat(self, interaction: discord.Interaction, prompt: str):
        ...
        if not HF_TOKEN:
            return await interaction.followup.send("❌ HF_TOKEN is missing!", ephemeral=True)
...
class HighTech(commands.Cog):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(API_URL, headers=headers, json=payload) as resp:
                    if resp.status == 200:
                        ...
                        embed = discord.Embed(
                            title="🧠 AI Response", 
                            description=ai_reply[:4000] if ai_reply else "No response",
                            color=0x9b59b6
                        )
    ...
    @app_commands.command(name="study", description="Start a 25-minute focus timer")
    async def study(self, interaction: discord.Interaction):
        view = StudyView(interaction.user)
        embed = discord.Embed(title="🍅 Pomodoro Timer", description="Click **Start** to focus for 25 minutes!", color=0xe74c3c)
        embed.add_field(name="Focus Subjects", value="📐 Maths | ⚛️ Physics | 🧪 Chemistry", inline=False)
        await interaction.response.send_message(embed=embed, view=view)
...
class StudyView(discord.ui.View):
    ...
    async def run_timer(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="⏰ Time's Up!", description=f"Great job {self.user.mention}!", color=0x2ecc71)
        await interaction.followup.send(embed=embed)
        self.is_running = False
async def setup(bot):
